...
def showItemDetail(item_id):
    items = session.query(Item).filter_by(
        id=item_id).one()
    if 'username' not in login_session:
        return render_template('publicitemdetail.html', items=items)
    else:
...
def editItem(item_id):
    ...
    if login_session['user_id'] != editedItem.user_id:
        flash('''You are not authorized to edit this item.
              Please create your own items in order to edit items.''')
...
def deleteItem(item_id):
    ...
    if login_session['user_id'] != itemToDelete.user_id:
        flash('''You are not authorized to delete this item.
              Please create your own items in order to delete items.''')
        return redirect(url_for('showItemDetail', item_id=item_id))
    if request.method == 'POST':
